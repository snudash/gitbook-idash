...
프로그래밍을 하면서 터미널을 사용할 일이 많기 때문에 Visual Studio Code의 경우에 에디터 내부에서 셸을 열 수 있는 기능이 있습니다. 웹 페이지를 만드는 경우에는 터미널과 함께 쓸 일이 많지 않기는 합니다. Linux나 macOS의 경우에는 별도의 터미널을 쓰는 것이 편할 수도 있지만 Windows에서는 Visual Studio Code 내에서 터미널이 더 좋을 수 있습니다. Visual Studio Code에서 Command Palette를 열고 "Terminal: Select Default  Shell" 명령을 선택하고 기본 셸을 선택하세요. WSL, Cygwin, GitBash 등 설치한 것 중에 선택하시면 됩니다.
